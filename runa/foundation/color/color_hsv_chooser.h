...
namespace runa
{
    ...
    using arg_color_hsv_chooser = arg_color_chooser;
	...
	namespace drawerbase
	{
		namespace color_hsv_chooser
		{
			...
			public:
                ...
                constexpr static uint v_height = 20;
    ...
    inline void operator<<(color_hsv_chooser& _os, const string& _v)
    {
        _os.value(_v);
    }
    inline void operator>>(const color_hsv_chooser& _is, string& _v)
    {
        _v = _is.value().str();
    }

    inline void operator<<(color_hsv_chooser& _os, const color& _v)
    {
        _os.value(_v);
    }
    inline void operator>>(const color_hsv_chooser& _is, color& _v)
    {
        _v = _is.value();
    }
